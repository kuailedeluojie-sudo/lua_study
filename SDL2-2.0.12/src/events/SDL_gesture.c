...
void SDL_GestureProcessEvent(SDL_Event* event)
{
    ...
    if (event->type == SDL_FINGERMOTION ||
        ...
        x = event->tfinger.x * 1000;
        y = event->tfinger.y * 1000;
        ...
        if (event->type == SDL_FINGERUP) {
            ...
            float dx = event->tfinger.dx * 1000;
            float dy = event->tfinger.dy * 1000;
#if defined(ENABLE_DOLLAR)
            ...
            inTouch->centroid.y += dy/inTouch->numDownFingers;
             printf("Centrid : (%f,%f)\n",inTouch->centroid.x,inTouch->centroid.y);            if (inTouch->numDownFingers > 1) {
